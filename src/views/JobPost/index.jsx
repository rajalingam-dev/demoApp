...
/** ***************************** Import Action ******************************** */
import { GetJobPostList } from "../../redux/actions";
...
const JobPost = (props) => {
  ...
  const initialUserData = async () => {
    ...
    dispatch(GetJobPostList()).then((res) => {
  ...
  const getUsersAfterDelete = (currentPage) => {
    ...
    // dispatch(GetJobPostList({ currentPage, rowsPerPage, sortBy })).then((res) => {
      dispatch(GetJobPostList()).then((res) => {
      if (responseHandler(res)) {
        setContactMessage(res.data);
        setTotalResults(res.totalResults);
  ...
  const getUsersAfterAdd = async (currentPage) => {
    ...
    dispatch(GetJobPostList()).then((res) => {
      // dispatch(GetJobPostList(data)).then((res) => {
        console.log("response2 delete", res);
      setContactMessage(res?.data);
      setTotalResults(res?.totalResults);
      setTotalPages(res.totalPages);
      setCurrentPage(currentPage);
  ...
  console.log("contactMessage111111", contactMessage);
