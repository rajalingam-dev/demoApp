...
const Carrier = (props) => {
  ...
  useEffect(() => {
      ...
      setCarrierData(changeFieldNames);
    // eslint-disable-next-line
  }, [ props.ContactMessage ]);
  // const user = carrierData.find((user) => user.id.toString() === props.CandidateId);
  // const user = carrierData.find((user) => console.log(user, props.CandidateId));

  const userDetails = carrierData
    ? Object.entries(carrierData) 
    : [["id", <span>No Records Found</span>]];
  console.log("Check User Details:", userDetails, "User:", carrierData);
  return (
    <CRow>
      <CCol lg={ 12 }>
        <CCard>
