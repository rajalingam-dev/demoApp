...
/** ***************************** Import Components ******************************** */
import CategoryList from "./blogSettingList";
import formatDate from "../../../utils/dateFormat";

/** **************************** Import API ****************************** */

/** ***************************** Import Action ******************************** */
import { GetblogPostSettingList } from "../../../redux/actions";
...
const BlogSetting = (props) => {
  ...
  const initialUserData = async () => {
    ...
    dispatch(GetblogPostSettingList()).then((res) => {
      if (res) {
        console.log("response123", res.page);
  ...
  const nextPage = (currentPage) => {
    ...
    dispatch(GetblogPostSettingList({
      data,
    })).then((res) => {
  ...
  const getUsersAfterDelete = (currentPage) => {
    ...
    dispatch(GetblogPostSettingList({ currentPage, rowsPerPage, sortBy })).then((res) => {
      if (responseHandler(res)) {
        setCategory(res.results);
        setTotalResults(res.totalResults);
        setApiPage(res.page);
  ...
  const getUsersAfterAdd = async (currentPage) => {
    ...
    // dispatch(GetblogPostSettingList(data)).then((res) => {
      dispatch(GetblogPostSettingList()).then((res) => {
      setCategory(res.data);
      setTotalResults(res.totalResults);
      setTotalPages(res.totalPages);
