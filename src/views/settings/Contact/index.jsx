...
const Contact = (props) => {
  ...
  const initialUserData = async () => {
    const data = {};
    dispatch(GetContactMessageList(data)).then((res) => {
      if (res) {
  ...
  const getUsersAfterDelete = (currentPage) => {
    ...
    GetContactMessageList({ currentPage, rowsPerPage, sortBy }).then((res) => {
      if (responseHandler(res)) {
        setContactMessage(res.results);
        setTotalResults(res.totalResults);
        setApiPage(res.page);
  ...
  const getUsersAfterAdd = async (currentPage) => {
    ...
    await props.GetContactMessageList(data).then((res) => {
      setContactMessage(res.results);
      setTotalResults(res.totalResults);
      setTotalPages(res.totalPages);
      setCurrentPage(currentPage);
